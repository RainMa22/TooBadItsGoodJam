...
SettingsBox newSettingsBox(Rectangle bounds, int thickness, Color lineColor)
{
    ...
    Dragbar volume = newDragbar((Rectangle){volumePos.x, volumePos.y, bounds.width * 6 / 8, bounds.height / 5},
    ...
    Dragbar narration = newDragbar((Rectangle){volumePos.x, volumePos.y, bounds.width * 6 / 8, bounds.height / 5},
                                   (Vector2){bounds.width / 16, bounds.height / 6}, 100, DARKGRAY, GRAY);

    Vector2 yapVolPos = relativeToAbsolute(selfPos, (Vector2){bounds.width / 8, bounds.height * 2 / 5});
    Dragbar yapVol = newDragbar((Rectangle){volumePos.x, volumePos.y, bounds.width * 6 / 8, bounds.height / 5},
                                (Vector2){bounds.width / 16, bounds.height / 6}, 100, DARKGRAY, GRAY);
